...
OPCS3_cols <- paste("X41273.0.",0:15,sep="")
OPCS4_cols <- paste("X41272.0.",0:116,sep="")
...
ukb_data_processed %>% 
  ...
  ##
  unite(col=OPCS3,OPCS3_cols,sep=",",remove=TRUE) %>%
  unite(col=OPCS4,OPCS4_cols,sep=",",remove=TRUE) %>%
  mutate(OPCS3=paste0(",",OPCS3,",")) %>%
  mutate(OPCS4=paste0(",",OPCS4,",")) %>%
  mutate(ovariesremoved_OPCS3=(grepl(c(",681.2,|,691,|,692,|,692.1,"),OPCS3))) %>%
  mutate(ovariesremoved_OPCS4=(grepl(c(",Q221,|,Q222,|,Q223,|,Q228,|,Q229,|,Q232,|,Q236,"),OPCS4))) %>%
  mutate(hysterectomy_OPCS3=(grepl(c(",691,|,692|,693|,694|,696"),OPCS3))) %>%
  mutate(hysterectomy_OPCS4=(grepl(c(",Q07|,Q08"),OPCS4))) %>%
  ##
  ##
